...
export const DragPlugin = {
  ...
  init(element, config = {}) {
    ...
    const handleMouseMove = (event) => {
      ...
      // Clamp position to stay within viewport (0-100%)
      // Consider element dimensions later for more precise clamping
      ...
      const currentScale = element.container.style.transform.match(/scale\(([^)]+)\)/);
    ...
    const handleMouseUp = (event) => {
      ...
      if (finalX !== elementStartX || finalY !== elementStartY) {
          ...
          element.updateElementState({ position: { x: finalX, y: finalY } });
      } else {
      ...
      attachEventListeners: () => {
        if (element.container) {
          ...
          // element.container.addEventListener('touchstart', handleTouchStart);
            ...
            console.error(`DragPlugin: Cannot attach listener, element container not found for ${element.id}`);
        }
      },
